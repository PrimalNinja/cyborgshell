...
pb(b):(D=b[1].trim(),l("Project name set to: "+D,!1,!0),m("cmdProject"))):(0<D.length?l("Current project: "+D,!1,!0):l("No project name set.",!1,!0),m("cmdProject"));break;case "renum":d[h].cd=y(C(d[h].cd,!1),d[h].mte);m("cmdRenum");break;case "reset":Qa(!1);break;case "rtl":qa(!0);break;case "run":Sa(b,b);break;case "touch":1<b.length?(b=w(b[1].trim(),10),0>b?k("Invalid file number."):b>=t?k("File does not exist."):(d[b].dt=!0,Ja(b),U(b))):k("Missing file number.");m("cmdTouch");break;case "type":sb(b);
break;case "width":f=$("<span>").css({"font-family":F.css("font-family"),"font-size":F.css("font-size"),visibility:"hidden"}).text("W");F.append(f);b=f.width();f.remove();f=F.width();l(Math.floor(f/b),!1,!0);break;case "notes":case "startup":z?k("Invalid operation for the local space."):s(a,1,vb);break;case "login":Oa(b);break;case "validatecookie":s(a,0,wb,c);break;case "spaces":s(a,0,ub,!1,!0);break;case "cat":case "dir":case "ls":f=a;if(z){f=[];1<b.length&&b[1];for(b=0;b<localStorage.length;b++)a=
localStorage.key(b),"string"===typeof a&&(0<a.length&&a.startsWith(R))&&(a=a.substring(R.length),f.push(a));0===f.length?k("No files found."):(f.sort(),b=f.join(", "),l(b,!1,!0));m("cmdDir")}else s(f,0);break;case "device":case "devices":case "language":case "languages":case "about":case "admins":case "alias":case "chpwd":case "password":case "logout":case "offline":case "online":case "register":case "username":case "users":case "grant":case "keys":case "newkey":case "revoke":case "shares":s(a,0);
break;case "cd":case "chdir":case "cs":case "space":f=a;1<b.length&&"local"===b[1].trim().toLowerCase()?(ha?(z=!0,l("Space is now local.",!1,!0)):k("Local storage unavailable."),m("cmdSpace")):1<b.length?(z=!1,s(f,1)):z?(l("local",!1,!0),m("cmdSpace")):s(f,1);break;case "del":case "delete":case "era":z?k("Invalid operation for the local space."):s(a,1);break;case "help":s(a,1);break;case "load":z?k("Invalid operation for the local space."):s(a,1,null,!1,!1,!1,!0);break;case "saveall":rb();break;case "save":z?
k("Invalid operation for the local space."):s(a,1,null,!1,!1,!1,!1,!0);break;case "copy":case "cp":case "mv":case "ren":case "rename":z?k("Invalid operation for the local space."):s(a,2);break;default:b=a,f=b.split(" "),-1!==f[0].indexOf("(")||-1!==f[0].indexOf(")")?ta(b):(f[0].endsWith(".js")||(f[0]+=".js"),f.unshift("run"),Sa(f,f))}else kb(g,a,c)}var ib=!1,Ma=!0,T="csdevicekey",tb=365,lb=3,Ba=1,nb=7,L="config-",R="cyborgshell-",va=48,mb=1E3,H="Unnamed File",fb="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~",
